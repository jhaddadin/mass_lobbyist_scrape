...
$masterlist.insert(0, ["id", "name", "year", "lobbyistrefid", "lobbyistname", "lobbyistamount", "lobbyistemployed", "lobbyistterminated"])

# Save the values to a CSV file
CSV.open("data/entities#{$year}_lobbyists.csv","w") do |csv|
$masterlist.each { |row|
  csv << row
}
end