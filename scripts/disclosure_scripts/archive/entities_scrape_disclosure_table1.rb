...
CSV.open("data/entity_disclosure_table1_#{$year}.csv","w") do |csv|
$masterlist.each { |row|
  csv << row
}
end
...
if $errorlog != []
  CSV.open("data/entity_disclosure_table1_#{$year}_errorlog.csv","w") do |csv|
    $errorlog.each { |row|
     csv << row
   }
  end
...
if $changed != []
  CSV.open("data/entity_disclosure_table1_#{$year}_changelog.csv","w") do |csv|
    $changed.each { |row|
      csv << row
    }
  end
