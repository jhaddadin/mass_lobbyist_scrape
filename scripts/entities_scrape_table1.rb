...
def tablescrape(id,htmlfile)
  ...
  if @type == "Entity"
    ...
    # Salary
    @salary = @htmlpage.xpath("//*[@id='ContentPlaceHolder1_RptClient_lblTotalExpense']").text
    ...
    # Clean up @salary, @expenses by removing commas and dollar signs
    ...
    @subarray = [@id, @name, @year, @type, @address, @totalsalarypaid, @salary, @expenses, @op_expenses, @mealstravel_expenses, @addl_expenses]
    $masterlist << @subarray
...
$masterlist.insert(0, ["id", "name", "year", "type", "address", "total_salary_paid", "total_salary_received", "expenses", "op_expenses", "mealstravel_expenses", "addl_expenses"])

# Save the values to a CSV file
CSV.open("data/entities#{$year}_table1.csv","w") do |csv|
$masterlist.each { |row|
  csv << row
}
end