...
def tablescrape(id,htmlfile)
  ...
  if @type == "Lobbyist"
    ...
    @subarray = [@id, @name, @year, @type, @address, @salary, @expenses, @op_expenses, @mealstravel_expenses, @addl_expenses]
    $masterlist << @subarray
...
$masterlist.insert(0, ["id", "name", "year", "type", "address", "salary", "expenses", "op_expenses", "mealstravel_expenses", "addl_expenses"])

# Save the values to a CSV file
CSV.open("data/lobbyists#{$year}_table1.csv","w") do |csv|
$masterlist.each { |row|
  csv << row
}
end